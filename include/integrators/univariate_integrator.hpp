...
// Factory functions for common integrator configurations
template<concepts::Field T>
constexpr auto make_gauss_integrator(std::size_t order = 5) {
    if (order == 3) {
        return quadrature_integrator<T, quadrature::gauss_legendre<T, 3>>{
            quadrature::gauss_legendre<T, 3>{}
        };
    } else {
        return quadrature_integrator<T, quadrature::gauss_legendre<T, 5>>{
            quadrature::gauss_legendre<T, 5>{}
        };
    }
}

template<concepts::Field T>
constexpr auto make_adaptive_integrator() {
    using rule = quadrature::gauss_kronrod_15<T>;
    using acc = accumulators::neumaier_accumulator<T>;
    return quadrature_integrator<T, rule, acc>{rule{}, acc{}};
}

template<concepts::Field T>
constexpr auto make_tanh_sinh() {
    return tanh_sinh_integrator<T>{};
}

template<concepts::Field T>
constexpr auto make_romberg() {
