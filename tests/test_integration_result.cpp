...
TEST(IntegrationResultPerformance, DISABLED_ManyOperations) {
    const size_t n = 1000000;

    integration_result<double> total;

    auto start = std::chrono::high_resolution_clock::now();

    for (size_t i = 0; i < n; ++i) {
        integration_result<double> partial(
    ...
    std::cout << "Accumulated " << n << " results in " << duration.count() << " ms\n";
    std::cout << "Final value: " << total.value() << ", error: " << total.error()
              << ", iterations: " << total.iterations() << "\n";

    // Check correctness
    double expected_sum = static_cast<double>(n - 1) * n / (2.0 * n);
    EXPECT_NEAR(total.value(), expected_sum, expected_sum * 1e-10);
}